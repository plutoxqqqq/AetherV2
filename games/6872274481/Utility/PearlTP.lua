run(function()
	...
	task.spawn(function()
		...
		if remote and remote.instance then projectileRemote = remote.instance end
	end)
	...
	local function throwPearl(target)
		...
		local aim = prediction.SolveTrajectory(origin, meta.launchVelocity, meta.gravitationalAcceleration, target, Vector3.zero, workspace.Gravity, 0, 0)
		if not aim then
		...
		local projectile = bedwars.ProjectileController:createLocalProjectile(meta, 'telepearl', 'telepearl', origin, nil, direction, {drawDurationSeconds = 1})
		local success = pcall(function()
			projectileRemote:InvokeServer(
				pearl.tool, 'telepearl', 'telepearl', origin, origin, direction,
		...
		if not success and projectile then projectile:Destroy() end
