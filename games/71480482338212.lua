...
run(function()
    ...
    local function fireProjectileAt(ent)
        ...
        if not (entitylib.isAlive and source) then return false end

        local ammo = source.ammoItemTypes and source.ammoItemTypes[1] or hand.Name
        ...
        local target = prediction.SolveTrajectory(selfpos, speed, projmeta.gravitationalAcceleration or 196.2, ent.RootPart.Position, ent.RootPart.Velocity, workspace.Gravity, ent.HipHeight, nil, RaycastParams.new(), nil, lplr:GetNetworkPing()) or ent.RootPart.Position
        ...
        local success = pcall(function()
            -- Spawn the client-side projectile with the SAME id the server call
            -- uses so the arrow is visible and the launch animates; firing the
            -- bare remote showed nothing on the local client.
            ...
            bedwars.Client:Get(remotes.FireProjectile):CallServerAsync(hand, ammo, projectile, shootPosition, selfpos, dir * speed, id, {
    ...
    Projectile = TriggerBot:CreateToggle({Name = 'Projectiles'})
    ProjectileRange = TriggerBot:CreateSlider({Name = 'Projectile Range', Min = 10, Max = 120, Default = 60, Suffix = 'studs'})
    Targets = TriggerBot:CreateTargets({Players = true, NPCs = true})
    ProjectileBlacklist = TriggerBot:CreateTextList({Name = 'Projectile Blacklist', Default = {'telepearl', 'fireball'}})
end)
                            ...
                            if Mode.Value == 'Normal' then
                                ...
                                if top then
                                    ...
                                    connection = runService.PreSimulation:Connect(function()
                                        if vape.Modules.Fly.Enabled or InfiniteFly.Enabled or vape.Modules['LongJump'].Enabled then
...
run(function()
    ...
    local function getGround(root)
        rayCheck.FilterDescendantsInstances = {lplr.Character, gameCamera, AntiFallPart}
        return workspace:Raycast(root.Position, Vector3.new(0, -500, 0), rayCheck)
    end

    local function getMapBottom()
        local bottom = math.huge
        for _, block in store.blocks do
            bottom = math.min(bottom, block.Position.Y - (block.Size.Y / 2))
        end
        return bottom ~= math.huge and bottom - 18 or -100
    end
                ...
                repeat
                    ...
                    if entitylib.isAlive then
                        ...
                        if root and humanoid and root.AssemblyLinearVelocity.Y < -65 then
                            local ray = getGround(root)
                            if Mode.Value == 'TP' and ray then
                                local old = root.CFrame
                                root.CFrame = CFrame.new(old.X, math.min(ray.Position.Y - 12, getMapBottom()), old.Z)
                                task.wait()
                                root.AssemblyLinearVelocity = Vector3.zero
                                root.CFrame = CFrame.new(old.X, ray.Position.Y + entitylib.character.HipHeight + 2.5, old.Z)
                                waitDelay = 0.18
                            elseif not ray or (root.Position.Y - ray.Position.Y) > 5 then
...
run(function()
    ...
    local function projectileAttribute(part, attribute)
        local current = part
        for _ = 1, 3 do
    ...
    local function isProjectile(part)
        ...
        if projectileAttribute(part, 'ProjectileShooter') == lplr.UserId then return false end
        local name = part.Name:lower()
        local parentName = part.Parent and part.Parent.Name:lower() or ''
        return projectileAttribute(part, 'Projectile') or projectileAttribute(part, 'ProjectileShooter') or projectileAttribute(part, 'ProjectileType') or name:find('arrow') or name:find('projectile') or name:find('fireball') or name:find('telepearl') or parentName:find('arrow') or parentName:find('projectile')
    end

    local function getProjectileVelocity(part)
        local now = os.clock()
        local history = projectileHistory[part]
        projectileHistory[part] = {Position = part.Position, Time = now}
        local velocity = part.AssemblyLinearVelocity
        if velocity.Magnitude > 1 then return velocity end
        if history and now > history.Time then
            return (part.Position - history.Position) / (now - history.Time)
        end
        return Vector3.zero
    end

    local function incomingProjectile(root)
        for part in projectileCache do
            if not part.Parent then
                projectileCache[part] = nil
                projectileHistory[part] = nil
                continue
            end
            if (part.Position - root.Position).Magnitude <= math.max(Range.Value, 70) then
                local velocity = getProjectileVelocity(part)
                if velocity.Magnitude > 2 then
                    local toLocal = root.Position - part.Position
                    if toLocal.Magnitude > 0 then
                        local closing = velocity.Unit:Dot(toLocal.Unit)
                        local closestTime = math.clamp(toLocal:Dot(velocity) / velocity:Dot(velocity), 0, 1.25)
                        local closestPoint = part.Position + velocity * closestTime
                        local miss = (root.Position - closestPoint).Magnitude
                        if closing > 0.15 and closestTime <= 1.25 and miss < 14 then return {Part = part, TimeToHit = closestTime, Velocity = velocity} end
                    end
                end
            end
        end
    ...
    local function hasProjectilePassed(threat, root)
        local part = threat and threat.Part
        if not part or not part.Parent or not root then return true end
        local velocity = getProjectileVelocity(part)
    ...
    AntiDeath = vape.Categories.Blatant:CreateModule({
        ...
        Function = function(call)
            if call then
                ...
                AntiDeath:Clean(workspace.DescendantAdded:Connect(function(obj)
                    if isProjectile(obj) then projectileCache[obj] = true end
                end))
                ...
                repeat
                    if entitylib.isAlive then
                        ...
                        -- Cover reaction latency plus the network round-trip: the hitbox
                        -- move must replicate to the server before it counts.
                        local reactionBuffer = 0
                        pcall(function() reactionBuffer = math.min(lplr:GetNetworkPing() + 0.05, 0.35) end)
...
run(function()
    ...
    local MoveProtection
    local Debug

    local oldroot, clone, hip
    local dodgeActive, dodgeType, recoverToken = false, nil, 0
    local lastDodgeEnd = 0
    local projectileCache, projectileHistory, threatProjectiles = {}, {}, {}
    local rayParams = RaycastParams.new()
    rayParams.FilterType = Enum.RaycastFilterType.Include
    rayParams.RespectCanCollide = true

    local function getRoot()
        return entitylib.isAlive and (entitylib.character.RootPart or entitylib.character.HumanoidRootPart)
    end
    ...
    local function refreshProjectileCache()
        table.clear(projectileCache)
        table.clear(projectileHistory)
        table.clear(threatProjectiles)
        for _, obj in workspace:GetDescendants() do
            if isProjectile(obj) then projectileCache[obj] = true end
        end
    end
    ...
    local function hasProjectilePassed(threat, root)
        ...
        if velocity.Magnitude <= 2 then return true end
        local toLocal = root.Position - part.Position
        return velocity:Dot(toLocal) <= 0 or toLocal.Magnitude > ProjectileRange.Value
    end

    local function incomingProjectile(root)
        local bestThreat, bestTime = nil, math.huge
        table.clear(threatProjectiles)
        for part in projectileCache do
            if not part.Parent then
                projectileCache[part] = nil
            ...
            if (part.Position - root.Position).Magnitude <= ProjectileRange.Value then
                local velocity = getProjectileVelocity(part)
                if velocity.Magnitude > 2 then
                    local toLocal = root.Position - part.Position
                    if toLocal.Magnitude > 0.001 then
                        local velocityDot = velocity:Dot(velocity)
                        local timeToHit = velocityDot > 0 and math.clamp(toLocal:Dot(velocity) / velocityDot, 0, 2) or 2
                        local closestPoint = part.Position + velocity * timeToHit
                        local missDistance = (root.Position - closestPoint).Magnitude
                        local closing = velocity.Unit:Dot(toLocal.Unit)
                        if closing > 0.1 and missDistance < 16 then
                            local threat = {Part = part, TimeToHit = timeToHit, MissDistance = missDistance, Velocity = velocity}
                            threatProjectiles[part] = threat
                            if timeToHit < bestTime then
                                bestThreat, bestTime = threat, timeToHit
                            end
                        end
                    end
                end
        ...
        return bestThreat and bestTime <= math.max(ProjectileStretch.Value, 0.05) and bestThreat or nil
    end

    local function mapFloorY(x, z)
        local lowestY = 0
        local foundBlock = false
        for _, block in store.blocks do
            lowestY = foundBlock and math.min(lowestY, block.Position.Y - block.Size.Y / 2) or block.Position.Y - block.Size.Y / 2
            foundBlock = true
        end
        local startY = (foundBlock and lowestY or -20) - 10
        local ray = workspace:Raycast(Vector3.new(x, startY, z), Vector3.new(0, 1000, 0), rayParams)
        return ray and ray.Position.Y or (foundBlock and lowestY or 0)
    end

    local function getFarBelowPosition(position)
        return CFrame.new(position.X, math.max(mapFloorY(position.X, position.Z) - 6, -80), position.Z) * CFrame.Angles(math.rad(90), 0, 0)
    end

    local function getSafeOffset(cframe, direction)
        local safeDirection = direction.Magnitude > 0.001 and direction.Unit or Vector3.new(1, 0, 0)
        local targetPosition = cframe.Position + safeDirection * OffsetDistance.Value
        targetPosition = Vector3.new(targetPosition.X, math.max(targetPosition.Y, -80), targetPosition.Z)

        local rayDown = workspace:Raycast(targetPosition, Vector3.new(0, -5, 0), rayParams)
        if rayDown and rayDown.Instance.CanCollide then targetPosition += Vector3.new(0, 5, 0) end
        local rayUp = workspace:Raycast(targetPosition, Vector3.new(0, 5, 0), rayParams)
        if rayUp and rayUp.Instance.CanCollide then targetPosition -= Vector3.new(0, 5, 0) end
        return CFrame.new(targetPosition)
    end

    local function wouldSuffocate(part)
        if not AntiSuffocation.Enabled then return false end
        for _, touchingPart in workspace:GetPartBoundsInBox(part.CFrame, part.Size + Vector3.new(0.25, 0.25, 0.25)) do
            if touchingPart ~= part and touchingPart.CanCollide and touchingPart.Transparency < 0.5 and not touchingPart:IsDescendantOf(lplr.Character) then
                return true
            end
        end
        return false
    ...
    local function doClone()
        ...
        hip = entitylib.character.Humanoid.HipHeight
        oldroot = entitylib.character.HumanoidRootPart
        if not oldroot or not lplr.Character.Parent then return false end
        lplr.Character.Parent = replicatedStorage
        clone = oldroot:Clone()
        clone.Name = 'AntiDeathPlus_Clone'
        clone.CanCollide = DodgeMode.Value ~= 'NoClipGhost'
        clone.Parent = lplr.Character
        oldroot.Transparency = 1
        oldroot.Parent = workspace
        store.rootpart = oldroot
    ...
    local function applyDodge(threatDirection)
        if not dodgeActive or not oldroot or not clone then return end
        local mode = DodgeMode.Value
        if mode == 'NoClipGhost' then
            clone.CanCollide = false
            if MoveProtection.Enabled then
                clone.Velocity = entitylib.character.Humanoid.MoveDirection * 16
            end
            if wouldSuffocate(clone) then
                local escape = clone.Position - oldroot.Position
                clone.CFrame += (escape.Magnitude > 0.001 and escape.Unit or Vector3.new(0, 1, 0)) * 2
            end
        end

        if mode == 'FarTeleport' then
            oldroot.CFrame = getFarBelowPosition(clone.Position)
        else
            oldroot.CFrame = getSafeOffset(clone.CFrame, threatDirection or Vector3.new(1, 0, 0))
        end
        oldroot.Velocity = Vector3.zero
    end

    local function beginDodge(dodgeReason, threat)
        if dodgeActive or (os.clock() - lastDodgeEnd) < Cooldown.Value or not doClone() then return end
        dodgeActive = true
        dodgeType = dodgeReason
        recoverToken += 1
        local token = recoverToken
        if Debug.Enabled then notif('AntiDeathPlus', 'Dodging ' .. dodgeReason, 2, 'info') end
        task.spawn(function()
            if dodgeReason == 'projectile' then
                local started = tick()
                repeat
                    task.wait()
                until token ~= recoverToken or not AntiDeathPlus.Enabled or hasProjectilePassed(threat, getRoot()) or tick() - started > DodgeDuration.Value
            else
                task.wait(DodgeDuration.Value)
            end
            if token == recoverToken and AntiDeathPlus.Enabled then revertClone() end
        end)
    end
    ...
    AntiDeathPlus = vape.Categories.Blatant:CreateModule({
        ...
        Function = function(call)
            if call then
                ...
                if not AntiDeathPlus.Enabled then return end

                refreshProjectileCache()
                AntiDeathPlus:Clean(workspace.DescendantAdded:Connect(function(obj)
                    if isProjectile(obj) then projectileCache[obj] = true end
                end))
                rayParams.FilterDescendantsInstances = {store.map}

                AntiDeathPlus:Clean(runService.PostSimulation:Connect(function()
                    if not dodgeActive or not oldroot or not clone then return end
                    local threatDirection
                    if dodgeType == 'melee' then
                        local enemy = entitylib.EntityPosition({Range = MeleeRange.Value, Players = Targets.Players.Enabled, NPCs = Targets.NPCs.Enabled, Wallcheck = Targets.Walls.Enabled or nil, Sort = sortmethods.Distance, Part = 'RootPart'})
                        if enemy and (enemy.Position - clone.Position).Magnitude > 0.001 then threatDirection = (enemy.Position - clone.Position).Unit end
                    elseif next(threatProjectiles) then
                        local closestDistance, closestThreat = math.huge, nil
                        for _, threat in threatProjectiles do
                            local distance = (threat.Part.Position - clone.Position).Magnitude
                            if distance < closestDistance then closestDistance, closestThreat = distance, threat end
                        end
                        if closestThreat and closestThreat.Velocity.Magnitude > 0.001 then threatDirection = -closestThreat.Velocity.Unit end
                    end
                    applyDodge(threatDirection)
                end))

                local lastOwnership = true
                repeat
                    if entitylib.isAlive then
                        if oldroot then
                            local ownership = isnetworkowner(oldroot)
                            if not ownership and ownership ~= lastOwnership then notif('AntiDeathPlus', 'Network ownership lost', 7, 'alert') end
                        ...
                        local root = getRoot()
                        if root then
                            local projectileThreat = Projectiles.Enabled and incomingProjectile(root)
                            local meleeThreat = Melee.Enabled and entitylib.EntityPosition({Range = MeleeRange.Value, Players = Targets.Players.Enabled, NPCs = Targets.NPCs.Enabled, Wallcheck = Targets.Walls.Enabled or nil, Sort = sortmethods.Distance, Part = 'RootPart'})
                            if projectileThreat then
                                beginDodge('projectile', projectileThreat)
                            elseif meleeThreat then
                                beginDodge('melee')
                            end
                        end
                    else
                    ...
                    task.wait()
                until not AntiDeathPlus.Enabled
            else
                revertClone()
            end
        end,
        ExtraText = function() return DodgeMode.Value end
    })
    ...
    AntiSuffocation = AntiDeathPlus:CreateToggle({Name = 'Anti-Suffocation', Default = true})
    MoveProtection = AntiDeathPlus:CreateToggle({Name = 'Movement Protection', Default = true})
    Debug = AntiDeathPlus:CreateToggle({Name = 'Debug Messages', Default = false})
end)
...
run(function()
    ...
    local function teleportDodge(root, side)
        local distance = TeleportDistance.Value
    ...
    local function incomingDirection(part, root, range)
        if (part.Position - root.Position).Magnitude > math.max(range, 70) then return end
        local velocity = getProjectileVelocity(part)
        ...
        local closestPoint = part.Position + velocity * closestTime
        local miss = (root.Position - closestPoint).Magnitude
        if closing > 0.15 and closestTime <= 1.25 and miss < 14 then
            return safeDirection(root, velocity.Unit:Cross(Vector3.yAxis)), closestTime
        end
    end
    ...
    ProjectileDodger = vape.Categories.Blatant:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                ProjectileDodger:Clean(workspace.DescendantAdded:Connect(function(obj)
                    if isProjectile(obj) then
                        projectiles[obj] = true
                    end
                end))
                ...
                repeat
                    if entitylib.isAlive then
                        local root = entitylib.character.RootPart
                        if tick() < dodgeUntil and Mode.Value == 'Legit' then
                            -- Bounded per-tick CFrame step only. Adding to
                            -- AssemblyLinearVelocity every 0.03s compounded to ~15x
                            -- Strength over the dodge window and flung the player.
                            root.CFrame += dodgeDirection * (math.clamp(Strength.Value, 10, 80) * 0.03)
                        else
                            for obj in projectiles do
                                ...
                                -- threat appears let the player drift back into the arc while
                                -- the per-projectile lockout blocked a second dodge.
                                local ping = 0
                                pcall(function() ping = lplr:GetNetworkPing() end)
                                local reactionWindow = math.min(ping + (Mode.Value == 'Teleport' and 0.25 or 0.45), 0.6)
                                if side and (timeToHit or 0) <= reactionWindow and (not dodgedProjectiles[obj] or tick() - dodgedProjectiles[obj] > 1.5) then
                                    dodgeDirection = side
                                    if Mode.Value == 'Teleport' then
                                        if teleportDodge(root, side) then
                                            dodgedProjectiles[obj] = tick()
                                            dodgeUntil = tick() + math.clamp(timeToHit or 0.2, 0.12, 0.35)
                                        end
                                    else
                                        dodgedProjectiles[obj] = tick()
                                        dodgeUntil = tick() + math.clamp((timeToHit or 0.3) + 0.15, 0.25, 0.6)
                                    end
                                    break
                                end
                            end
    ...
    Strength = ProjectileDodger:CreateSlider({Name = 'Dodge Strength', Min = 10, Max = 80, Default = 38, Suffix = 'studs'})
    ...
    TPAura = vape.Categories.Blatant:CreateModule({
        ...
        Function = function(callback)
            if callback then
                repeat
                    if entitylib.isAlive then
                        local ent = entitylib.EntityPosition({Range = Range.Value, Players = Targets.Players.Enabled, NPCs = Targets.NPCs.Enabled, Wallcheck = Targets.Walls.Enabled or nil, Part = 'RootPart', Sort = sortmethods.Distance})
                        if isValidTarget(ent) then
                            ...
                            for i = 1, 14 do
                                ...
                                local angle = ((i / 14) * math.pi * 2) + rand:NextNumber(-0.18, 0.18)
                                local distance = rand:NextNumber(math.max(3, radius - 1.25), radius)
                                position = getGroundPosition(targetPosition + Vector3.new(math.cos(angle) * distance, 0, math.sin(angle) * distance), targetPosition)
                            end

                            if position then
                                entitylib.character.RootPart.CFrame = CFrame.lookAt(position, targetPosition)
...
run(function()
    ...
    -- ProjectileLanding should preview the shot for the whole time a bow/throwable is
    -- readied, not only after the arrow has already left the string. Gating on
    -- MouseButton1 was unreliable - the draw input isn't always reported while the
    -- game holds the mouse locked, and it never triggered in third person - so instead
    -- we preview whenever a projectile weapon is actually held. Drawing the bow is a
    -- subset of holding it, so the estimate is now visible for the entire aim.
    ...
    local function getAimingLanding()
        ...
        -- the game's own fire path. Crucially we DON'T bail when ProjectileMeta has no
        -- entry: some bows resolve to a key that isn't in the table, and hard-returning
        -- there is what left the aim preview blank while the arc still showed up fine
        -- once the arrow was airborne. Keep sensible defaults so a marker is always drawn.
        local meta = (projectileType and bedwars.ProjectileMeta[projectileType]) or bedwars.ProjectileMeta[ammo]
        local speed = (meta and meta.launchVelocity) or source.launchVelocity or 100
        local gravity = (meta and meta.gravitationalAcceleration) or workspace.Gravity
        -- Aim exactly like the game's own fire path: resolve the crosshair's world target
        -- from the CAMERA - correct in both first and third person - then aim FROM THE
        -- ROOT towards that point and spawn from the bow muzzle offset along the same look.
        -- Deriving the direction from the muzzle instead of the root skewed the arc a
        -- couple of studs, worst in third person where the muzzle sits far from the camera.
        rayParams.FilterDescendantsInstances = {lplr.Character, gameCamera}
        local camCFrame = gameCamera.CFrame
        local camRay = workspace:Raycast(camCFrame.Position, camCFrame.LookVector * 5000, rayParams)
        local aimPoint = camRay and camRay.Position or (camCFrame.Position + camCFrame.LookVector * 5000)
        local rootPos = root.Position
        local direction = aimPoint - rootPos
        direction = direction.Magnitude > 1e-3 and direction.Unit or camCFrame.LookVector
    							...
    							for _, data in getProjectiles() do
    								if (FireRate[data[1].itemType] or 0) < tick() then
    									...
    									if hotbar and old and hotbarSwitch(hotbar) then
    										local ignore = vape.Modules['SilentAura'].Enabled or not inputService.MouseEnabled
    										task.wait(Delay.Value)
    										...
    										shootFunc(true)
    										if vape.Modules['AutoClicker'].Enabled and not ignore then
...
run(function()
    ...
    local function preserveMomentum(controller, momentum)
        if type(momentum) ~= 'number' then return end
        if type(controller.momentum) == 'number' and controller.momentum >= momentum then return end
        controller.momentum = momentum
        controller.lastMomentumReport = momentum
        if momentumRemote then
            pcall(function()
                momentumRemote:SendToServer({ momentumValue = momentum })
            end)
        end
    end
            ...
            if callback then
                ...
                if not oldUpdateMomentum then
                    ...
                    controller.updateMomentum = function(self, ...)
                        ...
                        preserveMomentum(self, momentum)
                        return result
                    end
                end
            else
    ...
    AutoBank = vape.Categories.Inventory:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                addItem('rageblade', false)
                addItem('emerald_sword', false)
                addItem('diamond_sword', false)
                addItem('telepearl', true)
                addItem('fireball', true)
    					...
    					for _, drill in getDrills(tagged) do
    						...
    						then
    							if collectDrill(drill) and Notify.Enabled then
    								notif('AutoDrill', 'Collected drill resources', 4, 'info')
    							end
