...
if isfile(path) then
	...
	if type(fetch) ~= 'function' then
		error('[AetherV2] Private source fetcher is unavailable for BedWars child-place forwarding', 0)
	end
	local ok, result = pcall(fetch, path, 3)
	if not ok or type(result) ~= 'string' or result == '' then
		error('[AetherV2] Failed to load canonical game '..targetPlace..': '..tostring(result), 0)
	end
	source = result
...
if not chunk then
	error('[AetherV2] Canonical game '..targetPlace..' failed to compile: '..tostring(compileError), 0)
end
