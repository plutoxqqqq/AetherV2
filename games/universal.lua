		...
		if callback then
			...
			for i, v in Toggles do
				if v.Toggle.Enabled then
					-- Guard object creation and every property write. Previously a single
					-- bad value (an empty string forced through ton()/tonumber, or an invalid
					-- skybox url) threw and aborted the entire build, which is why the module
					-- appeared to do nothing at all.
					local created, obj = pcall(Instance.new, i)
					if created and obj then
						for i2, v2 in v.Objects do
							pcall(function()
								if v2.Type == 'ColorSlider' then
									obj[i2] = Color3.fromHSV(v2.Hue, v2.Sat, v2.Value)
								elseif apidump[i][i2] == 'Number' then
