...
run(function()
    ...
    LowHealthVignette = vape.Categories.Legit:CreateModule({
        ...
        Function = function(enabled)
            ...
            LowHealthVignette:Clean(runService.RenderStepped:Connect(function()
                ...
                if character and humanoid then
                    ...
                    if ratio <= threshold then
                        ...
                        if Pulse.Enabled then
                            strength *= 0.75 + math.sin(os.clock() * 6) * 0.25
                        end
                    end
                end

                local warningColor = Color3.fromHSV(WarningColor.Hue, WarningColor.Sat, WarningColor.Value)
                for _, edge in edges do
                    edge.BackgroundColor3 = warningColor
                    edge.BackgroundTransparency = 1 - strength
                end

                heartbeatSound.Volume = strength > 0 and Heartbeat.Enabled and strength / 3 or 0
...
run(function()
    local Keystrokes, Style, Color, ShowSpace, ShowMouse, ShowCPS
    local keys, holder, clicks = {}, nil, {L = {}, R = {}}
    ...
    local function build()
        for _, key in keys do key.Key:Destroy() end; table.clear(keys)
        ...
        if ShowMouse.Enabled then
            make('MouseL', UDim2.fromOffset(118, 0), UDim2.fromOffset(29, 51), 'L', Enum.UserInputType.MouseButton1)
            make('MouseR', UDim2.fromOffset(153, 0), UDim2.fromOffset(29, 51), 'R', Enum.UserInputType.MouseButton2)
            make('MouseM', UDim2.fromOffset(147, 57), UDim2.fromOffset(12, 21), '', Enum.UserInputType.MouseButton3)
            if ShowCPS.Enabled then
                make('CPSL', UDim2.fromOffset(118, 83), UDim2.fromOffset(29, 24), '0', nil)
                make('CPSR', UDim2.fromOffset(153, 83), UDim2.fromOffset(29, 24), '0', nil)
            end
        end
        layout()
    end
    ...
    Keystrokes = vape.Categories.Legit:CreateModule({Name = 'Keystrokes', Category = 'Hud', Size = UDim2.fromOffset(110, 107), Tooltip = 'Shows movement, spacebar, mouse buttons, and CPS onscreen', Function = function(enabled)
        ...
        Keystrokes:Clean(inputService.InputBegan:Connect(function(input, processed)
            for id, entry in keys do if entry.Input == input.KeyCode or entry.Input == input.UserInputType then illuminate(entry, true); if not processed and id == 'MouseL' then table.insert(clicks.L, tick()) elseif not processed and id == 'MouseR' then table.insert(clicks.R, tick()) end end end
        end))
        Keystrokes:Clean(inputService.InputEnded:Connect(function(input) for _, entry in keys do if entry.Input == input.KeyCode or entry.Input == input.UserInputType then illuminate(entry, false) end end end))
        Keystrokes:Clean(runService.RenderStepped:Connect(function()
            local now = tick(); for _, side in {'L','R'} do while clicks[side][1] and now - clicks[side][1] > 1 do table.remove(clicks[side], 1) end end
            if keys.CPSL then keys.CPSL.Label.Text = tostring(#clicks.L); keys.CPSR.Label.Text = tostring(#clicks.R) end
        end))
    end})
