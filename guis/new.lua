...
local function updateTargetInfoPosition(target)
	...
	for _, offset in offsets do
		local center = target2d + offset
		local clamped = Vector2.new(
			math.clamp(center.X, half.X + margin, viewport.X - half.X - margin),
			math.clamp(center.Y, half.Y + margin, viewport.Y - half.Y - margin)
		)
		-- Prefer the candidate furthest from both combatants. The clamp penalty prevents
		-- an off-screen candidate from winning and then snapping on top of either player.
		local clearance = math.min((clamped - target2d).Magnitude, (clamped - localPoint).Magnitude)
		local score = clearance - ((clamped - center).Magnitude * 2)
		if not bestScore or score > bestScore then best, bestScore = clamped, score end
	end
