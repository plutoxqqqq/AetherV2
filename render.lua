...
run(function()
	...
	bedwars.breakBlock = function(block, effects, anim, customHealthbar)
		...
		if pos then
			...
			-- Always resolve the most efficient tool for this block (shears for wool,
			-- axe for the bed frame, pickaxe for stone, ...). getBreakTool searches the
			-- inventory for uncached break types like the bed frame, so an axe is used
			-- even when it isn't one of the pre-cached tools. If no ideal tool exists for
			-- this break type, fall back to the pickaxe so we never silently mine with
			-- whatever leftover item happens to be in hand.
			...
			if tool then
				...
				-- interrupted, and keep the bow while ProjectileAura is mid-shot so its shot
				-- isn't yanked away -- but never leave the WRONG break tool equipped: a
				-- leftover pickaxe on a wood bed must still switch to the axe. The old guard
				-- blocked every switch during combat, which is exactly how a pickaxe ended up
				-- breaking beds while KillAura/ProjectileAura/AntiDeath were active.
				local busyMelee = holdingSword and (now - bedwars.SwordController.lastAttack) <= 0.4
				local busyProjectile = holdingProjectile and (now - store.lastProjectileFire) <= 0.35
